...
export class ModalFund {
  ...
  listFunds: ListFunds[] = signal(this.data.listFunds ?? [])();
  ...
  changeFund() {
    if (this.data.type === 'add') {
      this.fundsForm.controls.idFund.valueChanges
        .subscribe((res) => {
          if (res) {
            const fund = this.listFunds.find((fund) => fund.idFund === Number(res));
  ...
  **/
  ...
  buyFund() {
    if (this.fundsForm.valid) {
      const total = this.fundsForm.value.total ?? 0;
      console.log('form', this.fundsForm.value);
      console.log(' this.data', this.data);
      // Compara el total con el monto actual del usuario
      if (total > this.data.currentAmount) {
        Swal.fire({
          icon: 'error',
      ...
      if (this.fundsForm.dirty) {
        Swal.fire({
          ...
          text: `Has comprado ${this.fundsForm.value.unitsFund} unidades del fondo
            ${this.fundsForm.value.nameFund} por un total de $${total.toLocaleString('es-ES')}.`,
        });
  ...
  addRegisterHistory() {
    const bodyHistory: HistoryFund = {
      ...
      unitsFund: Number(this.fundsForm.value.unitsFund),
      amountFund: Number(this.fundsForm.value.total),
      date: new Date(),
      subscription: '✅'
    };
